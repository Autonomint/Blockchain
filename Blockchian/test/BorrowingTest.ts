...
import { BorrowingTest, CDSTest, AMINTStablecoin, ABONDToken, Treasury,Options,USDT,MultiSign} from "../typechain-types";
import {
    wethGateway,
    priceFeedAddress,
    aTokenAddress,
...
describe("Borrowing Contract",function(){
    ...
    async function deployer(){
        ...
        await BorrowingContract.setAdmin(owner.address);
        return {Token,abondToken,usdt,CDSContract,BorrowingContract,treasury,options,multiSign,aToken,cETH,owner,user1,user2,user3,provider}
    }
    ...
    describe("Should revert errors",function(){
        it("Should revert if zero eth is deposited",async function(){
            const {BorrowingContract} = await loadFixture(deployer);
            const timeStamp = await time.latest();
            const tx =  BorrowingContract.connect(user1).depositTokens(100000,timeStamp,1,110000,ethVolatility,{value: ethers.utils.parseEther("0")});
        ...
        it("Should revert if LTV set to zero value before providing loans",async function(){
            ...
            await usdt.connect(user1).approve(CDSContract.address,10000000000);
            await CDSContract.connect(user1).deposit(10000000000,0,false,0);
            const tx =  BorrowingContract.connect(user1).depositTokens(100000,timeStamp,1,110000,ethVolatility,{value: ethers.utils.parseEther("1")});
        ...
        it("Should revert if the caller is not owner for initializeTreasury",async function(){
            ...
            await expect(BorrowingContract.connect(user1).initializeTreasury(treasury.address)).to.be.revertedWith("Ownable: caller is not the owner");
        })

        it("Should revert if the Treasury address is zero",async function(){
            const {BorrowingContract} = await loadFixture(deployer);
            await expect(BorrowingContract.connect(owner).initializeTreasury(ethers.constants.AddressZero)).to.be.revertedWith("Treasury must be contract address & can't be zero address");
        })
        ...
        it("Should revert if the caller is not owner for setOptions",async function(){
            ...
            await expect(BorrowingContract.connect(user1).setOptions(options.address)).to.be.revertedWith("Ownable: caller is not the owner");
        })

        it("Should revert if the Options address is zero",async function(){
            const {BorrowingContract} = await loadFixture(deployer);
            await expect(BorrowingContract.connect(owner).setOptions(ethers.constants.AddressZero)).to.be.revertedWith("Options must be contract address & can't be zero address");
        })

        it("Should revert if the caller is not owner for setAdmin",async function(){
            const {BorrowingContract,options} = await loadFixture(deployer);
            await expect(BorrowingContract.connect(user1).setAdmin(owner.address)).to.be.revertedWith("Ownable: caller is not the owner");
        })

        it("Should revert if the Admin address is zero",async function(){
            const {BorrowingContract} = await loadFixture(deployer);
            await expect(BorrowingContract.connect(owner).setAdmin(ethers.constants.AddressZero)).to.be.revertedWith("Admin can't be contract address & zero address");
        })
        ...
        it("Should revert if ratio is not eligible",async function(){
            ...
            await usdt.connect(user1).mint(user1.address,100000000)
            await usdt.connect(user1).approve(CDSContract.address,100000000);
            await CDSContract.connect(user1).deposit(100000000,0,true,ethers.utils.parseEther("50"));
            const tx = BorrowingContract.connect(user1).depositTokens(100000,timeStamp,1,110000,ethVolatility,{value: ethers.utils.parseEther("1")});
        ...
        //     await usdt.connect(user1).approve(CDSContract.address,10000000000);
        //     await CDSContract.connect(user1).deposit(10000000000,0,false,0);
        //     const tx = BorrowingContract.connect(ethers.constants.AddressZero).depositTokens(1000,timeStamp,1,110000,ethVolatility,{value: ethers.utils.parseEther("1")});
    ...
    describe("Should revert errors",function(){
        ...
        it("Should revert if the address is zero",async function(){
            const {treasury} = await loadFixture(deployer);
            await expect(treasury.connect(owner).setBorrowingContract(ethers.constants.AddressZero)).to.be.revertedWith("Input address is invalid");
        })

        it("Should revert if the address is invalid",async function(){
            const {treasury} = await loadFixture(deployer);
            await expect(treasury.connect(owner).setBorrowingContract(user1.address)).to.be.revertedWith("Input address is invalid");
        })

        it("Should revert if the caller is not owner",async function(){
            const {BorrowingContract,treasury} = await loadFixture(deployer);
            await expect(treasury.connect(user1).setBorrowingContract(BorrowingContract.address)).to.be.revertedWith("Ownable: caller is not the owner");
        })

        it("Should revert if the address is zero",async function(){
            const {treasury} = await loadFixture(deployer);
            await expect(treasury.connect(owner).withdrawInterest(ethers.constants.AddressZero,0)).to.be.revertedWith("Input address or amount is invalid");
        })

        it("Should revert if the caller is not owner",async function(){
            const {BorrowingContract,treasury} = await loadFixture(deployer);
            await expect(treasury.connect(user1).withdrawInterest(user1.address,100)).to.be.revertedWith("Ownable: caller is not the owner");
        })

        it("Should revert if Treasury don't have enough interest",async function(){
            const {BorrowingContract,treasury} = await loadFixture(deployer);
            await expect(treasury.connect(owner).withdrawInterest(user1.address,100)).to.be.revertedWith("Treasury don't have enough interest");
        })
    ...
    describe("Should update all state changes correctly",function(){
        it("Should update deposited amount",async function(){
            ...
            await expect(tx[0]).to.be.equal(ethers.utils.parseEther("1"))
        })
        ...
        it("Should update depositedAmount correctly if deposited multiple times",async function(){
            ...
            const tx = await treasury.borrowing(user1.address);
            await expect(tx[0]).to.be.equal(ethers.utils.parseEther("6"))
        })
        ...
        it("Should update totalVolumeOfBorrowersinUSD",async function(){
            ...
            await CDSContract.connect(user1).deposit(10000000000,0,true,ethers.utils.parseEther("5000"));
            await BorrowingContract.connect(user1).depositTokens(100000,timeStamp,1,110000,ethVolatility,{value: ethers.utils.parseEther("2")});
            await expect(await treasury.totalVolumeOfBorrowersAmountinUSD()).to.be.equal(ethers.utils.parseEther("200000"));
        })
        ...
        it("Should update totalVolumeOfBorrowersinUSD if multiple users deposit in different ethPrice",async function(){
            ...
            await expect(await treasury.totalVolumeOfBorrowersAmountinUSD()).to.be.equal(ethers.utils.parseEther("500000"));
        })
        ...
        it("Should update totalVolumeOfBorrowersinWei",async function(){
            ...
            await expect(await treasury.totalVolumeOfBorrowersAmountinWei()).to.be.equal(ethers.utils.parseEther("5"));
        })

        it("Should update borrowingContract",async () => {
            const {BorrowingContract,treasury} = await loadFixture(deployer);  
            await treasury.connect(owner).setBorrowingContract(BorrowingContract.address);
            await expect(await treasury.borrowingContract()).to.be.equal(BorrowingContract.address);
        })
    })
    ...
    describe("Should withdraw ETH from protocol",function(){
        it("Should withdraw ETH (between 0.8 and 1)",async function(){
            const {BorrowingContract,Token,abondToken,treasury,CDSContract} = await loadFixture(deployer);
            const timeStamp = await time.latest();
            await usdt.connect(user1).mint(user1.address,10000000000)
            await usdt.connect(user1).approve(CDSContract.address,10000000000);
            await CDSContract.connect(user1).deposit(10000000000,0,true,ethers.utils.parseEther("5000"));
            ...
            await abondToken.connect(user1).approve(BorrowingContract.address,await abondToken.balanceOf(user1.address));
            await BorrowingContract.connect(user1).withDraw(user2.address,1,99900,timeStamp,4);
        })
        ...
        it("Should revert To address is zero and contract address",async function(){
            ...
            const tx = BorrowingContract.connect(user1).withDraw(ethers.constants.AddressZero,1,99900,timeStamp,4);
            await expect(tx).to.be.revertedWith("To address cannot be a zero and contract address");

            const tx1 = BorrowingContract.connect(user1).withDraw(treasury.address,1,99900,timeStamp,4);
        ...
        it("Should revert if User already withdraw entire amount",async function(){
            const {BorrowingContract,Token,abondToken,CDSContract} = await loadFixture(deployer);
            const timeStamp = await time.latest();
            await usdt.connect(user1).mint(user1.address,10000000000)
            await usdt.connect(user1).approve(CDSContract.address,10000000000);
            await CDSContract.connect(user1).deposit(10000000000,0,true,ethers.utils.parseEther("5000"));
        ...
        it("Should revert User balance is less than required",async function(){
            const {BorrowingContract,Token,treasury} = await loadFixture(deployer);
            const timeStamp = await time.latest();
            await usdt.connect(user1).mint(user1.address,10000000000)
            await usdt.connect(user1).approve(CDSContract.address,10000000000);
            await CDSContract.connect(user1).deposit(10000000000,0,true,ethers.utils.parseEther("10000"));
            ...
            const tx = BorrowingContract.connect(user1).withDraw(user2.address,1,90000,timeStamp,4);
        ...
        it("Should revert Don't have enough Protocol Tokens",async function(){
            const {BorrowingContract,Token,abondToken} = await loadFixture(deployer);
            const timeStamp = await time.latest();
            await usdt.connect(user1).mint(user1.address,10000000000)
            await usdt.connect(user1).approve(CDSContract.address,10000000000);
            await CDSContract.connect(user1).deposit(10000000000,0,true,ethers.utils.parseEther("5000"));
            ...
            await Token.connect(user1).approve(BorrowingContract.address,await Token.balanceOf(user1.address));
            await BorrowingContract.connect(user1).withDraw(user2.address,1,99900,timeStamp,4);
            await abondToken.connect(user1).transfer(user2.address,ethers.utils.parseEther("0.1"));
    ...
    describe("Should Liquidate ETH from protocol",function(){
        ...
        it("Should revert Already liquidated",async function(){
            ...
            const tx = BorrowingContract.connect(owner).liquidate(user1.address,1,80000);
        ...
        it("Should revert if other than admin tried to Liquidate",async function(){
            ...
            const tx = BorrowingContract.connect(user2).liquidate(user1.address,1,80000);
            await expect(tx).to.be.revertedWith('Caller is not an admin');
        })

        it("Should revert To address is zero",async function(){
            const {BorrowingContract} = await loadFixture(deployer);
            const tx = BorrowingContract.connect(owner).liquidate(ethers.constants.AddressZero,1,100000);
        ...
        it("Should revert You cannot liquidate your own assets!",async function(){
            const {BorrowingContract} = await loadFixture(deployer);
            const tx = BorrowingContract.connect(owner).liquidate(owner.address,1,100000);
