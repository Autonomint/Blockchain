...
async function main() {
  ...
  const deployedBorrowing = await upgrades.upgradeProxy(Borrowing,[await deployedAMINTStablecoin.getAddress(),await deployedCDS.getAddress(),await deployedABONDToken.getAddress(),await deployedMultisign.getAddress(),priceFeedAddressGoerli,1],{initializer:'initialize'},{kind:'uups'});
  await deployedBorrowing.waitForDeployment();
  console.log("NEW IMP BORROWING ADDRESS",deployedBorrowing.address);
